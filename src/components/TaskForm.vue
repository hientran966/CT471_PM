  ...
  <div>
    <a-modal v-model:open="open" title="Thông tin dự án" ok-text="Lưu"
      cancel-text="Thoát" @ok="handleOk">
        <a-form :model="project" layout="vertical" @finish="handleOk">
          <a-form-item label="Tên dự án" name="tenDA" :rules="[{ required: true, message: 'Nhập tên dự án!' }]">
...
const rules: Record<string, Rule[]> = {
  ...
  ngayBD: [{ required: true, message: 'Xin chọn ngày bắt đầu', trigger: 'change', type: 'object' }],
  ngayKT: [
    { required: true, message: 'Xin chọn ngày bắt đầu', trigger: 'change', type: 'object' },
    {
      validator: async (_rule: Rule, value: Dayjs) => {
        ...
        if (formState.ngayBD && value.isBefore(formState.ngayBD)) {
